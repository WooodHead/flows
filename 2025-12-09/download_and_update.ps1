...
# ====== 创建桌面快捷方式 ======
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "检查桌面快捷方式..." -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

$exePath = Join-Path -Path $targetDir -ChildPath "BetterYeahCustomerRobot.exe"
$desktopPath = [Environment]::GetFolderPath("Desktop")
$shortcutPath = Join-Path -Path $desktopPath -ChildPath "BetterYeahCustomerRobot.lnk"

# 检查快捷方式是否存在
if (Test-Path -Path $shortcutPath) {
    Write-Host "[√] 桌面快捷方式已存在" -ForegroundColor Green
    ...
    if (Test-Path -Path $exePath) {
        try {
            ...
            $shortcut.Description = "BetterYeah Customer Robot"
            $shortcut.Save()
            ...
            Write-Host "    提示: 这不会影响程序的使用" -ForegroundColor Yellow
        }
    }
    else {
        Write-Host "[!] 可执行文件不存在，无法创建快捷方式" -ForegroundColor Yellow
